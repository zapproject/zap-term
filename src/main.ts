...
export class Main extends CLI{
    ...
    async start() {
        ...
        try{
            ...
            let provider = await loadProvider(this.web3, await loadAccount(this.web3));
            let subscriber = await loadSubscriber(this.web3, await loadAccount(this.web3));
            let providerCli = new ProviderCli(this.web3,provider)
            let subscriberCli = new SubscriberCli(this.web3,subscriber,bondage)
            let providerChoices = Object.keys(providerCli.list)
            let subscriberChoices = Object.keys(subscriberCli.list)
            let generalCli = new GeneralCli(this.web3,registry,bondage,dispatch)
            let generalChoices = Object.keys(generalCli.list)
            let templateCli = new TemplateCli(this.web3)
            ...
            console.log(" _____           \n" +
                ...
                "/____\\__,_| .__/ \n" +
                "          |_| ")
            console.log(await Util.viewInfo({web3:this.web3}))
            return await this.getMenu(this.mainMenu)
        }
        catch(e){
            console.error("Error Initilizing CLI tool: ",e)
        }
    }
